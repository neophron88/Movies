package com.rasulov.main.data.network_datasource.cache_info.sharedpreferences_impl

import android.app.Application
import android.content.Context
import com.google.gson.Gson
import com.rasulov.common.CurrentTime
import com.rasulov.main.data.network_datasource.cache_info.*
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext

private const val GENRE = "genre"

class GenreCacheInfoSharedPref(
    application: Application,
    private val currentTime: CurrentTime
) : CacheInfo<GenreValue, GenreCache> {

    private val pref = application.getSharedPreferences(GENRE, Context.MODE_PRIVATE)
    private val dispatcherIO = Dispatchers.IO

    override suspend fun getCacheStatus(value: GenreValue): Status =
        withContext(dispatcherIO) {
            val data = pref.getString(value.genreId.toString(), null)
                ?: return@withContext Revalidate()
            val cache = Gson().fromJson(data, GenreCache::class.java)
            if (cache.language != value.language) return@withContext Revalidate()
            if (cache.sortBy != value.sortBy) return@withContext Revalidate()
            cache.status(currentTime)
        }

    override suspend fun updateCache(cache: GenreCache) =
        withContext(dispatcherIO) {
            val data = Gson().toJson(cache)
            pref.edit().putString(cache.genreId.toString(), data).commit()
            Unit
        }

}

class GenreCache(
    val language: String,
    val sortBy: String,
    val genreId: Int,
    maxAge: Int?,
    eTag: String?,
    defaultMaxAgeIfOtherNull: Int,
    updatedTimeInSeconds: Long
) : Cache(maxAge, eTag, defaultMaxAgeIfOtherNull, updatedTimeInSeconds)

class GenreValue(
    val language: String,
    val sortBy: String,
    val genreId: Int,
) : Value()

