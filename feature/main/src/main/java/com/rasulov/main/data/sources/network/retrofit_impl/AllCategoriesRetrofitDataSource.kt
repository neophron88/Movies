...
import com.rasulov.main.data.sources.network.retrofit_impl.util.checkStatusAndLoadIfOutdated
import com.rasulov.main.data.sources.network.retrofit_impl.util.toAllGenresCache
import com.rasulov.main.data.sources.network.retrofit_impl.util.toGenreCache
import com.rasulov.main.data.sources.network.retrofit_impl.util.toQueryMap
...
class AllCategoriesRetrofitDataSource(
    ...
    override suspend fun loadMoviesByGenre(params: ByGenreNetworkParams): List<NetworkMovie> {
        val movies = checkStatusAndLoadIfOutdated(
            ...
            request = {
                service.loadMoviesByGenre(it.eTag, params.toQueryMap(genreInterceptor))
            },
            updateCacheInfo = {
    ...
    override suspend fun loadTopRated(params: BaseNetworkParams): List<NetworkMovie> =
        service.loadTopRated(null, params.toQueryMap()).body()?.results ?: emptyList()
